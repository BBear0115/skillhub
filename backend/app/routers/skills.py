...
def _build_skill_response(session, skill: Skill, workspace: Workspace | None = None) -> SkillResponse:
    ...
    runtime_ready = current_runtime_version(session, skill) is not None
...
async def list_skills(workspace_id: int, session: SessionDep, user: CurrentUserDep):
    ...
    return [_build_skill_response(session, skill, workspace) for skill in skills]


@router.get("/workspaces/{workspace_id}/agent-prompt", response_model=WorkspacePromptResponse)
async def build_workspace_agent_prompt(workspace_id: int, session: SessionDep, user: CurrentUserDep):
    if not await can_access_workspace(user, workspace_id):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
    workspace = session.get(Workspace, workspace_id)
    if not workspace:
    ...
    skill_rows = [
        {
            ...
            "tools": [tool.name for tool in _version_tools(session, version.id)],
        }
    ...
    prompt_text = "\n".join(
        [
            ...
            "Authentication: use Authorization: Bearer <workspace access token>.",
            "Available skills:",
            *skill_lines,
...
async def delete_skill(skill_id: int, session: SessionDep, user: CurrentUserDep):
    ...
    exposure = session.get(WorkspaceSkillExposure, (skill.workspace_id, skill.id))
    if exposure:
    ...
    for version in versions:
        ...
        cleanup_deployed_skill(skill.id, version.id)
    return {"ok": True}
